    ...
    for i in range(int(N_POPULATION / (N_BEST + N_CHILDREN))):
        for bg in best_genomes:
            ...
            if random.uniform(0, 1) < PROB_MUTATION:
                new_genome.w1 += new_genome.w1 * np.random.normal(mean, stddev, size=(6, 20)) / 100 * np.random.randint(-1, 2, (6, 20))
            if random.uniform(0, 1) < PROB_MUTATION:
                new_genome.w2 += new_genome.w2 * np.random.normal(mean, stddev, size=(20, 40)) / 100 * np.random.randint(-1, 2, (20, 40))
            if random.uniform(0, 1) < PROB_MUTATION:
                new_genome.w3 += new_genome.w3 * np.random.normal(mean, stddev, size=(40, 20)) / 100 * np.random.randint(-1, 2, (40, 20))
            if random.uniform(0, 1) < PROB_MUTATION:
                new_genome.w4 += new_genome.w4 * np.random.normal(mean, stddev, size=(20, 3)) / 100 * np.random.randint(-1, 2, (20, 3))
